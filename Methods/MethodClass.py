...
import datetime
import pandas as pd
from struct_building.CityStructure import borough
import matplotlib
...
class SituationMethods(FundamentalMethods):
    ...
    def briefSummary(self,Indicator,level,name='null'):
        ...
        if name == 'null':        
            print(self.IndicatorPrint[Indicator] + ' on the ' + level + ' level ' + ' is ' + str(sum))
            print(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>")
            
        else:
            print(self.IndicatorPrint[Indicator] + ' in ' + name + ' is ' + str(sum))
            print(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>")
    ...
    def RankTop10(self, Indicator, level, name='null'):    
        ...
        if name == 'null':
            ...
            colSumFrame = colSum.sort_values(ascending =False)
            print("The top 10 most dangerous are:")
            if int(len(colSumFrame.index)) >= 10 :
                print('>'.join(colSumFrame.head(n = 10).index))
                print(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>")
            else:
                print('>'.join(colSumFrame.index))
                print(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>")
